...
dir_results = op.join(dir_main, 'Malina_dissertation', 'additional analyses for the defense', '20Hz second harm', 'results')
...
harmonic = 2 #for the second harmonc
...
for study, conds in zip(studies, conds_all): 
    ...
    for f in files:
        ...
        for cond in conds:
            ...
            if np.any((cond == 'no80_1', cond == 'no80_2',
                      cond == 'lo80_1', cond == 'lo80_2',
                      cond == 'hi80_1', cond == 'hi80_2'), axis = 0):
                ASSRfreq = 81.92 * harmonic
                myfreqs_80 = fftfreq[np.logical_and(fftfreq > ASSRfreq-15, fftfreq < ASSRfreq+15)]
    
            if np.any((cond == 'no20_1', cond == 'no20_2',
                      cond == 'lo20_1', cond == 'lo20_2',
                      cond == 'hi20_1', cond == 'hi20_2'), axis = 0):
                ASSRfreq = 20.48 * harmonic
                myfreqs_20 = fftfreq[np.logical_and(fftfreq > ASSRfreq-15, fftfreq < ASSRfreq+15)]
    
            if np.any((cond == 'no1', cond == 'no2', cond == 'no3', cond == 'no4',
                       cond == 'lo1', cond == 'lo2', cond == 'lo3', cond == 'lo4',
                       cond == 'hi1', cond == 'hi2', cond == 'hi3', cond == 'hi4',
                       cond == 'vh1', cond == 'vh2', cond == 'vh3', cond == 'vh4',
                       cond == 'no40_1', cond == 'no40_2',
                      cond == 'lo40_1', cond == 'lo40_2',
                      cond == 'hi40_1', cond == 'hi40_2'), axis = 0):
                ASSRfreq = 40.96 * harmonic
                myfreqs_40 = fftfreq[np.logical_and(fftfreq > ASSRfreq-15, fftfreq < ASSRfreq+15)]
            ...
            # myfreqs[indx_noise]
            # for 20 Hz: [14.08, 14.72, 15.36, 16.  , 16.64, 17.28, 17.92, 18.56, 22.4, 23.04, 23.68, 24.32, 24.96, 25.6 , 26.24, 26.88]
            # for 40 Hz: [34.56, 35.2 , 35.84, 36.48, 37.12, 37.76, 38.4 , 39.04, 42.88, 43.52, 44.16, 44.8 , 45.44, 46.08, 46.72, 47.36]
            # for 80 Hz: [75.52, 76.16, 76.8 , 77.44, 78.08, 78.72, 79.36, 80., 83.84, 84.48, 85.12, 85.76, 86.4 , 87.04, 87.68, 88.32]
    ...
    df.to_csv(op.join(dir_results, 'data_audio_all_channels_{}_2nd_harmonic.tsv'.format(study)), sep='\t', index=False)
    df = pd.DataFrame(Dsel)
    df.to_csv(op.join(dir_results, 'data_audio_signal_noise_{}_2nd_harmonic.tsv'.format(study)), sep='\t', index=False)
...
for fp in fps:
    ...
    ax[1].plot(myfreqs_40, tmplo, 'b-', label='Low')
    ax[1].plot(myfreqs_40, tmphi, 'r-', label='High')
    #ax[1].plot(myfreqs_40, Damp['lo'][fps.index(fp)], 'b-', label='Low')
    #ax[1].plot(myfreqs_40, Damp['hi'][fps.index(fp)], 'r-', label='High')
    ...
    ax[1].axvspan(min(myfreqs_40[indx_noise]), myfreqs_40[indx_noise[7]], color='gray', alpha=0.2, lw=0)
    ax[1].axvspan(myfreqs_40[indx_noise[8]], max(myfreqs_40[indx_noise]), color='gray', alpha=0.2, lw=0)
    ax[1].yaxis.set_major_formatter(FormatStrFormatter('%.2f'))
    ax[1].set_title("Amplitude spectrum")
    fig.suptitle("Subject "+str(fp))
    fig.tight_layout()
    plt.savefig(op.join(dir_results, 'figures', 'individual_aspctm', 'fp_{}_2ndharm_ASSR_study1.jpg'.format(fp)), format='jpg', dpi = 600)
    plt.savefig(op.join(dir_results, 'figures', 'individual_aspctm', 'fp_{}_2ndharm_ASSR_study1.pdf'.format(fp)), format='pdf', dpi = 600)
    plt.close(fig=None)
...
ax[1].yaxis.set_major_formatter(FormatStrFormatter('%.2f'))
fig.tight_layout()
fig.savefig(op.join(dir_results, 'figures', 'figure_average_aspctm_and_ERP_2ndharm_ASSR_study1.jpg'), format='jpg', dpi=600)
fig.savefig(op.join(dir_results, 'figures', 'figure_average_aspctm_and_ERP_2ndharm_ASSR_study1.pdf'), format='pdf', dpi=600)
plt.close(fig=None)
...
plt.plot(myfreqs_40, tmplo, 'b-', label='Low')
plt.plot(myfreqs_40, tmphi, 'r-', label='High')
...
#ax.set_ylim(top = 0.3)
ax.axvspan(min(myfreqs_40[indx_noise]), myfreqs_40[indx_noise[7]], color='gray', alpha=0.2, lw=0)
ax.axvspan(myfreqs_40[indx_noise[8]], max(myfreqs_40[indx_noise]), color='gray', alpha=0.2, lw=0)
fig.tight_layout()
fig.savefig(op.join(dir_results, 'figures', 'figure_audio_itc_2ndharm_ASSR_study1.jpg'), format='jpg', dpi=600)
fig.savefig(op.join(dir_results, 'figures', 'figure_audio_itc_2ndharm_ASSR_study1.pdf'), format='pdf', dpi=600)
plt.close(fig=None)
...
for fp in fps:
    ...
    ax[1].plot(myfreqs_40, tmpno, 'k-', label='No')
    ax[1].plot(myfreqs_40, tmplo, 'b-', label='Low')
    ax[1].plot(myfreqs_40, tmphi, '-', color = 'orange', label='High')
    ax[1].plot(myfreqs_40, tmpvh, 'r-', label='Very High')
    ax[1].legend(frameon=False)
    ax[1].set_xlabel('Frequencies (Hz)')
    ax[1].set_ylabel('Amplitudes (µV)')
    ax[1].axvspan(min(myfreqs_40[indx_noise]), myfreqs_40[indx_noise[7]], color='gray', alpha=0.2, lw=0)
    ...
    fig.suptitle("Subject "+str(fp))
    fig.tight_layout()
    plt.savefig(op.join(dir_results, 'figures', 'individual_aspctm', 'fp_{}_2ndharm_ASSR_study2.jpg'.format(fp)), format='jpg', dpi = 600)
    plt.savefig(op.join(dir_results, 'figures', 'individual_aspctm', 'fp_{}_2ndharm_ASSR_study2.pdf'.format(fp)), format='pdf', dpi = 600)
    plt.close(fig=None)
...
ax[1].yaxis.set_major_formatter(FormatStrFormatter('%.2f'))
fig.tight_layout()
fig.savefig(op.join(dir_results, 'figures', 'figure_average_aspctm_and_ERP_2ndharm_ASSR_study2.jpg'), format='jpg', dpi=600)
fig.savefig(op.join(dir_results, 'figures', 'figure_average_aspctm_and_ERP_2ndharm_ASSR_study2.pdf'), format='pdf', dpi=600)
plt.close(fig=None)
...
plt.plot(myfreqs_40, tmpno, 'k-', label='No')
plt.plot(myfreqs_40, tmplo, 'b-', label='Low')
plt.plot(myfreqs_40, tmphi, '-', color = 'orange', label='High')
plt.plot(myfreqs_40, tmpvh, 'r-', label='Very High')
plt.title('Intertrial coherence (itc)')
plt.legend(frameon=False)
plt.xlabel('Frequencies (Hz)')
plt.ylabel('itc')
...
ax.axvspan(myfreqs_40[indx_noise[8]], max(myfreqs_40[indx_noise]), color='gray', alpha=0.2, lw=0)
fig.tight_layout()
fig.savefig(op.join(dir_results, 'figures', 'figure_audio_itc_2ndharm_ASSR_study2.jpg'), format='jpg', dpi=600)
fig.savefig(op.join(dir_results, 'figures', 'figure_audio_itc_2ndharm_ASSR_study2.pdf'), format='pdf', dpi=600)
plt.close(fig=None)
...
for fp in fps:
    ...
    for f in range(3):
        ...
        ax[1].plot(tmp_myfreqs[f], tmpno, 'k-', label='No')
        ax[1].plot(tmp_myfreqs[f], tmplo, 'b-', label='Low')
        ax[1].plot(tmp_myfreqs[f], tmphi, 'r-', label='High')
        ax[1].legend(frameon=False)
        ax[1].set_xlabel('Frequencies (Hz)')
        ax[1].set_ylabel('Amplitudes (µV)')
        ax[1].spines['top'].set_visible(False)
        ...
        ax[1].axvspan(min(tmp_myfreqs[f][indx_noise]), tmp_myfreqs[f][indx_noise[7]], color='gray', alpha=0.2, lw=0)
        ax[1].axvspan(tmp_myfreqs[f][indx_noise[8]], max(tmp_myfreqs[f][indx_noise]), color='gray', alpha=0.2, lw=0)
        fig.suptitle("Subject "+str(fp))
        fig.tight_layout()
        plt.savefig(op.join(dir_results, 'figures', 'individual_aspctm', 'fp_{}_2ndharm_for_{}Hz_AM.jpg'.format(fp, tmp_names[f])), format='jpg', dpi = 600)
        plt.savefig(op.join(dir_results, 'figures', 'individual_aspctm', 'fp_{}_2ndharm_for_{}Hz_AM.pdf'.format(fp, tmp_names[f])), format='pdf', dpi = 600)
        plt.close(fig=None)
...
for f in range(3):
    ...
    ax[1].axvspan(tmp_myfreqs[f][indx_noise[8]], max(tmp_myfreqs[f][indx_noise]), color='gray', alpha=0.2, lw=0)
    fig.tight_layout()
    fig.savefig(op.join(dir_results, 'figures', 'figure_average_aspctm_and_ERP_2ndharm_for_{}Hz_N_{}.jpg'.format(tmp_names[f], len(fps))), format='jpg', dpi=600)
    fig.savefig(op.join(dir_results, 'figures', 'figure_average_aspctm_and_ERP_2ndharm_for_{}Hz_N_{}.pdf'.format(tmp_names[f], len(fps))), format='pdf', dpi=600)
    plt.close(fig=None)
...
for f in range(3):
    ...
    plt.plot(tmp_myfreqs[f], tmpno, 'k-', label='No')
    plt.plot(tmp_myfreqs[f], tmplo, 'b-', label='Low')
    plt.plot(tmp_myfreqs[f], tmphi, 'r-', label='High')
    plt.title('Intertrial coherence (itc)')
    plt.legend(frameon=False)
    plt.xlabel('Frequencies (Hz)')
    plt.ylabel('itc')
    ...
    ax.spines['right'].set_visible(False)
    #ax.set_ylim(top = 0.5)
    ax.axvspan(min(tmp_myfreqs[f][indx_noise]), tmp_myfreqs[f][indx_noise[7]], color='gray', alpha=0.2, lw=0)
    ax.axvspan(tmp_myfreqs[f][indx_noise[8]], max(tmp_myfreqs[f][indx_noise]), color='gray', alpha=0.2, lw=0)
    fig.tight_layout()
    fig.savefig(op.join(dir_results, 'figures', 'figure_audio_itc_2ndharm_for_{}_N_{}.jpg'.format(tmp_names[f],len(fps))), format='jpg', dpi=600)
    fig.savefig(op.join(dir_results, 'figures', 'figure_audio_itc_2ndharm_for_{}_N_{}.pdf'.format(tmp_names[f],len(fps))), format='pdf', dpi=600)
    plt.close(fig=None)
...
#        ax[1].spines['right'].set_visible(False)
#        ax[1].yaxis.set_major_formatter(FormatStrFormatter('%.2f'))
#        ax[1].set_title("Amplitude spectrum for {}-Hz ASSR, 2nd harmonic".format(tmp_names[f]))
#        #tmp_myfreqs[f][indx_noise[7]]
#        ax[1].axvspan(min(tmp_myfreqs[f][indx_noise]), tmp_myfreqs[f][indx_noise[7]], color='gray', alpha=0.2, lw=0)
#        ax[1].axvspan(tmp_myfreqs[f][indx_noise[8]], max(tmp_myfreqs[f][indx_noise]), color='gray', alpha=0.2, lw=0)
#        fig.suptitle("Subject "+str(fp))
#        fig.tight_layout()
#        plt.savefig(op.join(dir_results, 'figures', 'individual_aspctm', 'fp_{}_for_{}Hz_AM_2nd_harmonic.jpg'.format(fp, tmp_names[f])), format='jpg', dpi = 600)
#        plt.savefig(op.join(dir_results, 'figures', 'individual_aspctm', 'fp_{}_for_{}Hz_AM_2nd_harmonic.pdf'.format(fp, tmp_names[f])), format='pdf', dpi = 600)
#        plt.close(fig=None)
#
#
## plot grand mean amplitude spectrum by load
...
#         axes = ax[0])
#        #ylim=dict(eeg=[-6, 12])
#    ax[0].set_title('Mean ERP (top) and amplitude spectrum (bottom) for {}-Hz ASSR, 2nd harmonic (N = {})'.format(tmp_names[f], len(fps)))
#    ax[0].set_ylabel('Amplitudes (µV)')
#    ax[0].legend(frameon=False)
#    ax[0].yaxis.set_major_formatter(FormatStrFormatter('%.2f'))
#    
...
#    ax[1].axvspan(tmp_myfreqs[f][indx_noise[8]], max(tmp_myfreqs[f][indx_noise]), color='gray', alpha=0.2, lw=0)
#    fig.tight_layout()
#    fig.savefig(op.join(dir_results, 'figures', 'figure_average_aspctm_and_ERP_for_{}Hz_N_{}_2nd_harmonic.jpg'.format(tmp_names[f], len(fps))), format='jpg', dpi=600)
#    fig.savefig(op.join(dir_results, 'figures', 'figure_average_aspctm_and_ERP_for_{}Hz_N_{}_2nd_harmonic.pdf'.format(tmp_names[f], len(fps))), format='pdf', dpi=600)
#    plt.close(fig=None)
#
#
## plot grand mean itc by load
...
#    plt.plot(tmp_myfreqs[f], tmplo, 'b-', label='Low')
#    plt.plot(tmp_myfreqs[f], tmphi, 'r-', label='High')
#    plt.title('Intertrial coherence (itc) for {}-Hz ASSR, 2nd harmonic'.format(tmp_names[f]))
#    plt.legend(frameon=False)
#    plt.xlabel('Frequencies (Hz)')
#    plt.ylabel('itc')
#    ax.spines['top'].set_visible(False)
...
#    ax.axvspan(tmp_myfreqs[f][indx_noise[8]], max(tmp_myfreqs[f][indx_noise]), color='gray', alpha=0.2, lw=0)
#    fig.tight_layout()
#    fig.savefig(op.join(dir_results, 'figures', 'figure_audio_itc_for_{}_N_{}_2nd_harmonic.jpg'.format(tmp_names[f],len(fps))), format='jpg', dpi=600)
#    fig.savefig(op.join(dir_results, 'figures', 'figure_audio_itc_for_{}_N_{}_2nd_harmonic.pdf'.format(tmp_names[f],len(fps))), format='pdf', dpi=600)
#    plt.close(fig=None)
#
#
#
