...
#            fftamps = fftamps[:,freqsindx]
            ...
            fft_indEpo_all = fft_indEpo[:,freqsindx_all]
            fft_indEpo = fft_indEpo[:,freqsindx]
    
            Damp[cond].append(np.mean(fft_indEpo, axis = 0))
            Damp_all[cond].append(np.mean(fft_indEpo_all, axis = 0))
            Damp_indEpo[cond].append(np.mean(fft_indEpo, axis = 0))
    
            for c in range(0,len(chanpick)):
                for i in range(0,len(myfreqs)):
                    tmpall['Amp%s_%s_%.2f'%(cond, chanpick[c],myfreqs[i])] = fft_indEpo[c,i]
    
            # S, N, SNR, and SmN for all channels
            tmpS = fft_indEpo[:,indx_signal]
            tmpN = np.mean(fft_indEpo[:,indx_noise],axis=1)
            tmpSNR = tmpS/tmpN
            tmpS = np.mean(tmpS)
            tmpN = np.mean(tmpN)
    ...
    df.to_csv(op.join(dir_results, 'data_audio_all_channels_{}_beta_notlocked.tsv'.format(study)), sep='\t', index=False)
    df = pd.DataFrame(Dsel)
    df.to_csv(op.join(dir_results, 'data_audio_signal_noise_{}_beta_notlocked.tsv'.format(study)), sep='\t', index=False)
...
for fp in fps:
    ...
    fig.tight_layout()
    plt.savefig(op.join(dir_results, 'figures', 'individual_aspctm', 'fp_{}_beta_ASSR_study1_notlocked.jpg'.format(fp)), format='jpg', dpi = 600)
    plt.savefig(op.join(dir_results, 'figures', 'individual_aspctm', 'fp_{}_beta_ASSR_study1_notlocked.pdf'.format(fp)), format='pdf', dpi = 600)
    plt.close(fig=None)
...
ax[1].yaxis.set_major_formatter(FormatStrFormatter('%.2f'))
fig.tight_layout()
fig.savefig(op.join(dir_results, 'figures', 'figure_average_aspctm_and_ERP_beta_ASSR_study1_notlocked.jpg'), format='jpg', dpi=600)
fig.savefig(op.join(dir_results, 'figures', 'figure_average_aspctm_and_ERP_beta_ASSR_study1_notlocked.pdf'), format='pdf', dpi=600)
plt.close(fig=None)
...
fig.tight_layout()
fig.savefig(op.join(dir_results, 'figures', 'figure_audio_itc_beta_ASSR_study1_notlocked.jpg'), format='jpg', dpi=600)
fig.savefig(op.join(dir_results, 'figures', 'figure_audio_itc_beta_ASSR_study1_notlocked.pdf'), format='pdf', dpi=600)
plt.close(fig=None)
...
for fp in fps:
    ...
    fig.suptitle("Subject "+str(fp))
    fig.tight_layout()
    plt.savefig(op.join(dir_results, 'figures', 'individual_aspctm', 'fp_{}_beta_ASSR_study2_notlocked.jpg'.format(fp)), format='jpg', dpi = 600)
    plt.savefig(op.join(dir_results, 'figures', 'individual_aspctm', 'fp_{}_beta_ASSR_study2_notlocked.pdf'.format(fp)), format='pdf', dpi = 600)
    plt.close(fig=None)
...
ax[1].yaxis.set_major_formatter(FormatStrFormatter('%.2f'))
fig.tight_layout()
fig.savefig(op.join(dir_results, 'figures', 'figure_average_aspctm_and_ERP_beta_ASSR_study2_notlocked.jpg'), format='jpg', dpi=600)
fig.savefig(op.join(dir_results, 'figures', 'figure_average_aspctm_and_ERP_beta_ASSR_study2_notlocked.pdf'), format='pdf', dpi=600)
plt.close(fig=None)
...
ax.axvspan(myfreqs[indx_noise[8]], max(myfreqs[indx_noise]), color='gray', alpha=0.2, lw=0)
fig.tight_layout()
fig.savefig(op.join(dir_results, 'figures', 'figure_audio_itc_beta_ASSR_study2_notlocked.jpg'), format='jpg', dpi=600)
fig.savefig(op.join(dir_results, 'figures', 'figure_audio_itc_beta_ASSR_study2_notlocked.pdf'), format='pdf', dpi=600)
plt.close(fig=None)
...
for fp in fps:
    ...
    for f in range(3):
        ...
        fig.suptitle("Subject "+str(fp))
        fig.tight_layout()
        plt.savefig(op.join(dir_results, 'figures', 'individual_aspctm', 'fp_{}_beta_for_{}Hz_AM_notlocked.jpg'.format(fp, tmp_names[f])), format='jpg', dpi = 600)
        plt.savefig(op.join(dir_results, 'figures', 'individual_aspctm', 'fp_{}_beta_for_{}Hz_AM_notlocked.pdf'.format(fp, tmp_names[f])), format='pdf', dpi = 600)
        plt.close(fig=None)
...
for f in range(3):
    ...
    ax[1].axvspan(myfreqs[indx_noise[8]], max(myfreqs[indx_noise]), color='gray', alpha=0.2, lw=0)
    fig.tight_layout()
    fig.savefig(op.join(dir_results, 'figures', 'figure_average_aspctm_and_ERP_beta_for_{}Hz_N_{}_notlocked.jpg'.format(tmp_names[f], len(fps))), format='jpg', dpi=600)
    fig.savefig(op.join(dir_results, 'figures', 'figure_average_aspctm_and_ERP_beta_for_{}Hz_N_{}_notlocked.pdf'.format(tmp_names[f], len(fps))), format='pdf', dpi=600)
    plt.close(fig=None)
...
for f in range(3):
    ...
    ax.axvspan(myfreqs[indx_noise[8]], max(myfreqs[indx_noise]), color='gray', alpha=0.2, lw=0)
    fig.tight_layout()
    fig.savefig(op.join(dir_results, 'figures', 'figure_audio_itc_beta_for_{}_N_{}_notlocked.jpg'.format(tmp_names[f],len(fps))), format='jpg', dpi=600)
    fig.savefig(op.join(dir_results, 'figures', 'figure_audio_itc_beta_for_{}_N_{}_notlocked.pdf'.format(tmp_names[f],len(fps))), format='pdf', dpi=600)
    plt.close(fig=None)
