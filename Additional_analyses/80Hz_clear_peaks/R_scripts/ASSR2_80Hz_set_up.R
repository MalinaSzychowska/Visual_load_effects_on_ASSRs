...
getdiffs = function(dataset, iv){
  d = subset(dataset, select=c(paste0(iv,"no80Hz"), paste0(iv,"lo80Hz"), paste0(iv,"hi80Hz")))
  d$no80Hz = d[,1]
  d$lo80Hz = d[,2]
  d$hi80Hz = d[,3]
  d$"no-lo80Hz" = d[,1] - d[,2]
...
contrastBFs = function (dataset, iv){
  d = getdiffs(dataset, iv)
  d = d[,c(7:9)] 
  Mean = colMeans(d)
  Mean = round(Mean, 4)
